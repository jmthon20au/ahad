...
BOT_TOKEN = "8839060416:AAGznZF9qGq5xvJMHwrKt1LBfWffv_wMcYs"
ADMIN_ID = 6454550864  
...
def get_dates():
    ...
    try:
        ...
        if res:
            ...
            return f"ميلادي: {gregorian_date} | هجري: {hijri_date}"
...
DUAA_TEXT = """
<b>بِسْمِ اللهِ الرَّحْمَنِ الرَّحِيمِ</b>\n
...
def start_cmd(message):
    ...
    status_str = "مفعّل ✅" if is_active else "متوقف ❌"
    date_info = get_dates()
...
def test_send_cmd(message):
    ...
    for uid, uinfo in data.items():
        if uinfo.get("active"):
            ...
            msg = (
                f"🧪 <b>[رسالة اختبار سريعة - تنتهي خلال دقيقة]</b>\n"
                f"{DUAA_TEXT}\n\n"
...
def callback_handler(call):
    ...
    if call.data == "expired":
        bot.answer_callback_query(call.id, "عجباً، انتهى وقت التسجيل الخاص بهذه الجلسة!", show_alert=True)
        return
    ...
    if call.data == "toggle_active":
        ...
        if user["streak"] >= 40:
            user["cycles"] += 1
            user["streak"] = 0
            msg_suffix = "\n\n🎉 <b>مبروك! تم إكمال 40 يوماً بنجاح وتم بدء دورة جديدة!</b>"
        ...
        if now.hour == 5 and now.minute == 00:
            ...
            for uid, uinfo in data.items():
                if uinfo.get("active"):
                    ...
                    send_occurred = True
                    try:
                        bot.send_audio(int(uid), audio=AUDIO_URL, caption="✨ <b>هذا دعاء العهد صوتياً</b>", parse_mode="HTML")
                    except Exception as e:
                    ...
                    try:
                        ...
                        uinfo["last_sent_date"] = today_str  # تسشجيل أن هذا اليوم تم إرساله
                        save_data(data)
                    except Exception as e:
        ...
        if now.hour == 10 and now.minute == 58:
            ...
            for uid, uinfo in data.items():
                if uinfo.get("active"):
                    ...
                    if uinfo["last_read_date"] != today_str and uinfo["streak"] > 0:
                        uinfo["streak"] = 0
                        uinfo["last_reset_date"] = today_str  # تسجيل أن هذا اليوم تم تصفيته
                        save_data(data)
                        ...
                        if uinfo.get("last_msg_id"):
                            try:
                                ...
                                print(f"تعذر قفل زر الرسالة: {ex}")
                        ...
                        try:
                            ...
                            print(f"خطأ في إرسال إشعار التصفير: {e}")
